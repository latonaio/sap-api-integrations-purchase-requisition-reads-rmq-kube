...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-purchase-requisition-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(purchaseRequisition string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "PurchaseRequisitionItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemAccountData, "function": "PurchaseRequisitionItemAccount"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) ItemDeliveryAddress(purchaseRequisition, purchaseRequisitionItem string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "PurchaseRequisitionItemDeliveryAddress"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) ItemAccount(purchaseRequisition, purchaseRequisitionItem string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "PurchaseRequisitionItemAccount"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) PurchasingDocument(purchasingDocument, purchasingDocumentItem string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "PurchaseRequisitionItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
